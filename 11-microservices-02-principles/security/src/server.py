...
metrics = PrometheusMetrics(server, defaults_prefix=NO_PREFIX, buckets=[0.1, 0.5, 1, 1.5, 2],
                            default_labels={"app_name": "security"})
...
def login():
    ...
    print(f'Got request for logging in user "{login}" password "{password}"')
    ...
    return make_response(jsonify({'token': jwt.encode({'sub': login}, jwt_key, algorithm="HS256")}))
